...
def get_current_user(
    db_service: database_service_dependency, token: str = Depends(oauth2_scheme)
):
...
def check_user_access(user_id: str, current_user: current_user_dependency):
...
def require_resource_access(resource_type: ResourceType) -> Callable:
    ...
    def check_access(
        ...
        **path_params,  # This will receive all path parameters
        ...
        if resource_type == ResourceType.USER:
            ...
            if not case:
                raise HTTPException(
                    status_code=404, detail=f"{resource_type.value} not found"
                )
    ...
    check_access.__signature__ = inspect.signature(check_access).replace(
        parameters=[
            ...
            *[
                p
                for p in inspect.signature(check_access).parameters.values()
...
current_user_resource_access_dependency = Annotated[
    User, Depends(require_resource_access(ResourceType.USER))
]
